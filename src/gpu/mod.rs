...
impl<'a> Gpu<'a> {
    ...
    pub fn step(&mut self) {
        ...
        let new_mode = if cur_mode == Mode::VBlank {
            if self.htick == 0 {
    ...
    fn bg_win_color(&self, x: u8, y: u8, map: TileMap, set: TileSet) -> AlphaColor {
        let tile_map_x = x / 8;
        let tile_map_y = y / 8;
        let tile_x = x % 8;
        let tile_y = y % 8;
                ...
                match self.line_cache[y][i] {
                    ...
                    Some(other) => {
                        ...
                        if sprite.x_pos() < other_sprite.x_pos()
                            || (sprite.x_pos() == other_sprite.x_pos() && index < other)
                        {
                            ...
                            // it's checked above.
                            for j in (i..(l - 1)).rev() {
                                self.line_cache[y][j + 1] = self.line_cache[y][j];
                            }
        ...
        for &entry in self.line_cache[y as usize].iter() {
            match entry {
                ...
                Some(index) => {
                    ...
                    let pix = self.pix_color(tile, sprite_x as u8, sprite_y as u8, TileSet::Set1);
