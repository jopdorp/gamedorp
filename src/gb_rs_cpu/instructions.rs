...
///
/// `instruction` is an `fn (&mut self::Cpu)` used to emulate the
/// instruction.
///
...
fn next_byte(cpu: &mut self::Cpu) -> u8 {
    let pc = cpu.pc();
...
/// `pc` twice
fn next_word(cpu: &mut self::Cpu) -> u16 {
...
fn rst(cpu: &mut self::Cpu, addr: u16) {
    let pc = cpu.pc();
...
fn call_nz_nn(cpu: &mut self::Cpu) {
    let addr = next_word(cpu);
...
fn call_nc_nn(cpu: &mut self::Cpu) {
    let addr = next_word(cpu);
...
fn sub_and_set_flags(cpu: &mut self::Cpu, x: u8, y: u8) -> u8 {
...
fn sub_with_carry_and_set_flags(cpu: &mut self::Cpu, x: u8, y: u8) -> u8 {
    // Check for borrow using 32bit arithmetics
    let x = x as u32;
    let y = y as u32;
...
fn add_and_set_flags(cpu: &mut self::Cpu, x: u8, y: u8) -> u8 {
...
fn add_with_carry_and_set_flags(cpu: &mut self::Cpu, x: u8, y: u8) -> u8 {
    // Check for carry using 32bit arithmetics
    let x = x as u32;
    let y = y as u32;
    let carry = cpu.carry() as u32;
...
fn add_word_and_set_flags(cpu: &mut self::Cpu, x: u16, y: u16) -> u16 {
    // Check for carry using 32bit arithmetics
    let x = x as u32;
    let y = y as u32;
    let r = x.wrapping_add(y);
...
fn xor(cpu: &mut self::Cpu, n: u8, m: u8) -> u8 {
    let r = n ^ m;
...
/// Disable interrupts
fn di(cpu: &mut self::Cpu) {
    cpu.disable_interrupts();
}

/// Enable interrupts
fn ei(cpu: &mut self::Cpu) {
...
/// they are disabled.
fn halt(cpu: &mut self::Cpu) {
    cpu.halt();
}

/// Stop, blank the screen and wait for button press
fn stop(cpu: &mut self::Cpu) {
...
mod bitops {
    ...
    /// cpu flags.
    fn swap(cpu: &mut self::Cpu, v: u8) -> u8 {
        cpu.set_zero(v == 0);
        cpu.set_substract(false);
        cpu.set_halfcarry(false);
        cpu.set_carry(false);
    ...
    fn bit_a(cpu: &mut self::Cpu, bit: u8) {
        let a = cpu.a();
    ...
    /// Test `A` bit 0
    fn bit_a_0(cpu: &mut self::Cpu) {
        bit_a(cpu, 0);
    }

    /// Test `A` bit 1
    fn bit_a_1(cpu: &mut self::Cpu) {
        bit_a(cpu, 1);
    }

    /// Test `A` bit 2
    fn bit_a_2(cpu: &mut self::Cpu) {
        bit_a(cpu, 2);
    }

    /// Test `A` bit 3
    fn bit_a_3(cpu: &mut self::Cpu) {
        bit_a(cpu, 3);
    }

    /// Test `A` bit 4
    fn bit_a_4(cpu: &mut self::Cpu) {
        bit_a(cpu, 4);
    }

    /// Test `A` bit 5
    fn bit_a_5(cpu: &mut self::Cpu) {
        bit_a(cpu, 5);
    }

    /// Test `A` bit 6
    fn bit_a_6(cpu: &mut self::Cpu) {
        bit_a(cpu, 6);
    }

    /// Test `A` bit 7
    fn bit_a_7(cpu: &mut self::Cpu) {
        bit_a(cpu, 7);
    }

    /// Helper function to test bits in `B`
    fn bit_b(cpu: &mut self::Cpu, bit: u8) {
        let b = cpu.b();
    ...
    /// Test `B` bit 0
    fn bit_b_0(cpu: &mut self::Cpu) {
        bit_b(cpu, 0);
    }

    /// Test `B` bit 1
    fn bit_b_1(cpu: &mut self::Cpu) {
        bit_b(cpu, 1);
    }

    /// Test `B` bit 2
    fn bit_b_2(cpu: &mut self::Cpu) {
        bit_b(cpu, 2);
    }

    /// Test `B` bit 3
    fn bit_b_3(cpu: &mut self::Cpu) {
        bit_b(cpu, 3);
    }

    /// Test `B` bit 4
    fn bit_b_4(cpu: &mut self::Cpu) {
        bit_b(cpu, 4);
    }

    /// Test `B` bit 5
    fn bit_b_5(cpu: &mut self::Cpu) {
        bit_b(cpu, 5);
    }

    /// Test `B` bit 6
    fn bit_b_6(cpu: &mut self::Cpu) {
        bit_b(cpu, 6);
    }

    /// Test `B` bit 7
    fn bit_b_7(cpu: &mut self::Cpu) {
        bit_b(cpu, 7);
    }

    /// Helper function to test bits in `C`
    fn bit_c(cpu: &mut self::Cpu, bit: u8) {
        let c = cpu.c();
    ...
    /// Test `C` bit 0
    fn bit_c_0(cpu: &mut self::Cpu) {
        bit_c(cpu, 0);
    }

    /// Test `C` bit 1
    fn bit_c_1(cpu: &mut self::Cpu) {
        bit_c(cpu, 1);
    }

    /// Test `C` bit 2
    fn bit_c_2(cpu: &mut self::Cpu) {
        bit_c(cpu, 2);
    }

    /// Test `C` bit 3
    fn bit_c_3(cpu: &mut self::Cpu) {
        bit_c(cpu, 3);
    }

    /// Test `C` bit 4
    fn bit_c_4(cpu: &mut self::Cpu) {
        bit_c(cpu, 4);
    }

    /// Test `C` bit 5
    fn bit_c_5(cpu: &mut self::Cpu) {
        bit_c(cpu, 5);
    }

    /// Test `C` bit 6
    fn bit_c_6(cpu: &mut self::Cpu) {
        bit_c(cpu, 6);
    }

    /// Test `C` bit 7
    fn bit_c_7(cpu: &mut self::Cpu) {
        bit_c(cpu, 7);
    }

    /// Helper function to test bits in `D`
    fn bit_d(cpu: &mut self::Cpu, bit: u8) {
        let d = cpu.d();
    ...
    /// Test `D` bit 0
    fn bit_d_0(cpu: &mut self::Cpu) {
        bit_d(cpu, 0);
    }

    /// Test `D` bit 1
    fn bit_d_1(cpu: &mut self::Cpu) {
        bit_d(cpu, 1);
    }

    /// Test `D` bit 2
    fn bit_d_2(cpu: &mut self::Cpu) {
        bit_d(cpu, 2);
    }

    /// Test `D` bit 3
    fn bit_d_3(cpu: &mut self::Cpu) {
        bit_d(cpu, 3);
    }

    /// Test `D` bit 4
    fn bit_d_4(cpu: &mut self::Cpu) {
        bit_d(cpu, 4);
    }

    /// Test `D` bit 5
    fn bit_d_5(cpu: &mut self::Cpu) {
        bit_d(cpu, 5);
    }

    /// Test `D` bit 6
    fn bit_d_6(cpu: &mut self::Cpu) {
        bit_d(cpu, 6);
    }

    /// Test `D` bit 7
    fn bit_d_7(cpu: &mut self::Cpu) {
        bit_d(cpu, 7);
    }

    /// Helper function to test bits in `E`
    fn bit_e(cpu: &mut self::Cpu, bit: u8) {
        let e = cpu.e();
    ...
    /// Test `E` bit 0
    fn bit_e_0(cpu: &mut self::Cpu) {
        bit_e(cpu, 0);
    }

    /// Test `E` bit 1
    fn bit_e_1(cpu: &mut self::Cpu) {
        bit_e(cpu, 1);
    }

    /// Test `E` bit 2
    fn bit_e_2(cpu: &mut self::Cpu) {
        bit_e(cpu, 2);
    }

    /// Test `E` bit 3
    fn bit_e_3(cpu: &mut self::Cpu) {
        bit_e(cpu, 3);
    }

    /// Test `E` bit 4
    fn bit_e_4(cpu: &mut self::Cpu) {
        bit_e(cpu, 4);
    }

    /// Test `E` bit 5
    fn bit_e_5(cpu: &mut self::Cpu) {
        bit_e(cpu, 5);
    }

    /// Test `E` bit 6
    fn bit_e_6(cpu: &mut self::Cpu) {
        bit_e(cpu, 6);
    }

    /// Test `E` bit 7
    fn bit_e_7(cpu: &mut self::Cpu) {
        bit_e(cpu, 7);
    }

    /// Helper function to test bits in `H`
    fn bit_h(cpu: &mut self::Cpu, bit: u8) {
        let h = cpu.h();
    ...
    /// Test `H` bit 0
    fn bit_h_0(cpu: &mut self::Cpu) {
        bit_h(cpu, 0);
    }

    /// Test `H` bit 1
    fn bit_h_1(cpu: &mut self::Cpu) {
        bit_h(cpu, 1);
    }

    /// Test `H` bit 2
    fn bit_h_2(cpu: &mut self::Cpu) {
        bit_h(cpu, 2);
    }

    /// Test `H` bit 3
    fn bit_h_3(cpu: &mut self::Cpu) {
        bit_h(cpu, 3);
    }

    /// Test `H` bit 4
    fn bit_h_4(cpu: &mut self::Cpu) {
        bit_h(cpu, 4);
    }

    /// Test `H` bit 5
    fn bit_h_5(cpu: &mut self::Cpu) {
        bit_h(cpu, 5);
    }

    /// Test `H` bit 6
    fn bit_h_6(cpu: &mut self::Cpu) {
        bit_h(cpu, 6);
    }

    /// Test `H` bit 7
    fn bit_h_7(cpu: &mut self::Cpu) {
        bit_h(cpu, 7);
    }

    /// Helper function to test bits in `L`
    fn bit_l(cpu: &mut self::Cpu, bit: u8) {
        let l = cpu.l();
    ...
    /// Test `L` bit 0
    fn bit_l_0(cpu: &mut self::Cpu) {
        bit_l(cpu, 0);
    }

    /// Test `L` bit 1
    fn bit_l_1(cpu: &mut self::Cpu) {
        bit_l(cpu, 1);
    }

    /// Test `L` bit 2
    fn bit_l_2(cpu: &mut self::Cpu) {
        bit_l(cpu, 2);
    }

    /// Test `L` bit 3
    fn bit_l_3(cpu: &mut self::Cpu) {
        bit_l(cpu, 3);
    }

    /// Test `L` bit 4
    fn bit_l_4(cpu: &mut self::Cpu) {
        bit_l(cpu, 4);
    }

    /// Test `L` bit 5
    fn bit_l_5(cpu: &mut self::Cpu) {
        bit_l(cpu, 5);
    }

    /// Test `L` bit 6
    fn bit_l_6(cpu: &mut self::Cpu) {
        bit_l(cpu, 6);
    }

    /// Test `L` bit 7
    fn bit_l_7(cpu: &mut self::Cpu) {
        bit_l(cpu, 7);
    }

    /// Helper function to test bits in `[HL]`
    fn bit_mhl(cpu: &mut self::Cpu, bit: u8) {
        let hl = cpu.hl();
    ...
    /// Test `[HL]` bit 0
    fn bit_mhl_0(cpu: &mut self::Cpu) {
        bit_mhl(cpu, 0);
    }

    /// Test `[HL]` bit 1
    fn bit_mhl_1(cpu: &mut self::Cpu) {
        bit_mhl(cpu, 1);
    }

    /// Test `[HL]` bit 2
    fn bit_mhl_2(cpu: &mut self::Cpu) {
        bit_mhl(cpu, 2);
    }

    /// Test `[HL]` bit 3
    fn bit_mhl_3(cpu: &mut self::Cpu) {
        bit_mhl(cpu, 3);
    }

    /// Test `[HL]` bit 4
    fn bit_mhl_4(cpu: &mut self::Cpu) {
        bit_mhl(cpu, 4);
    }

    /// Test `[HL]` bit 5
    fn bit_mhl_5(cpu: &mut self::Cpu) {
        bit_mhl(cpu, 5);
    }

    /// Test `[HL]` bit 6
    fn bit_mhl_6(cpu: &mut self::Cpu) {
        bit_mhl(cpu, 6);
    }

    /// Test `[HL]` bit 7
    fn bit_mhl_7(cpu: &mut self::Cpu) {
        bit_mhl(cpu, 7);
    }
    ...
    fn res_a(cpu: &mut self::Cpu, bit: u8) {
        let a = cpu.a();
    ...
    /// Clear `A` bit 0
    fn res_a_0(cpu: &mut self::Cpu) {
        res_a(cpu, 0);
    }

    /// Clear `A` bit 1
    fn res_a_1(cpu: &mut self::Cpu) {
        res_a(cpu, 1);
    }

    /// Clear `A` bit 2
    fn res_a_2(cpu: &mut self::Cpu) {
        res_a(cpu, 2);
    }

    /// Clear `A` bit 3
    fn res_a_3(cpu: &mut self::Cpu) {
        res_a(cpu, 3);
    }

    /// Clear `A` bit 4
    fn res_a_4(cpu: &mut self::Cpu) {
        res_a(cpu, 4);
    }

    /// Clear `A` bit 5
    fn res_a_5(cpu: &mut self::Cpu) {
        res_a(cpu, 5);
    }

    /// Clear `A` bit 6
    fn res_a_6(cpu: &mut self::Cpu) {
        res_a(cpu, 6);
    }

    /// Clear `A` bit 7
    fn res_a_7(cpu: &mut self::Cpu) {
        res_a(cpu, 7);
    }

    /// Helper function to clear bits in `B`
    fn res_b(cpu: &mut self::Cpu, bit: u8) {
        let b = cpu.b();
    ...
    /// Clear `B` bit 0
    fn res_b_0(cpu: &mut self::Cpu) {
        res_b(cpu, 0);
    }

    /// Clear `B` bit 1
    fn res_b_1(cpu: &mut self::Cpu) {
        res_b(cpu, 1);
    }

    /// Clear `B` bit 2
    fn res_b_2(cpu: &mut self::Cpu) {
        res_b(cpu, 2);
    }

    /// Clear `B` bit 3
    fn res_b_3(cpu: &mut self::Cpu) {
        res_b(cpu, 3);
    }

    /// Clear `B` bit 4
    fn res_b_4(cpu: &mut self::Cpu) {
        res_b(cpu, 4);
    }

    /// Clear `B` bit 5
    fn res_b_5(cpu: &mut self::Cpu) {
        res_b(cpu, 5);
    }

    /// Clear `B` bit 6
    fn res_b_6(cpu: &mut self::Cpu) {
        res_b(cpu, 6);
    }

    /// Clear `B` bit 7
    fn res_b_7(cpu: &mut self::Cpu) {
        res_b(cpu, 7);
    }

    /// Helper function to clear bits in `C`
    fn res_c(cpu: &mut self::Cpu, bit: u8) {
        let c = cpu.c();
    ...
    /// Clear `C` bit 0
    fn res_c_0(cpu: &mut self::Cpu) {
        res_c(cpu, 0);
    }

    /// Clear `C` bit 1
    fn res_c_1(cpu: &mut self::Cpu) {
        res_c(cpu, 1);
    }

    /// Clear `C` bit 2
    fn res_c_2(cpu: &mut self::Cpu) {
        res_c(cpu, 2);
    }

    /// Clear `C` bit 3
    fn res_c_3(cpu: &mut self::Cpu) {
        res_c(cpu, 3);
    }

    /// Clear `C` bit 4
    fn res_c_4(cpu: &mut self::Cpu) {
        res_c(cpu, 4);
    }

    /// Clear `C` bit 5
    fn res_c_5(cpu: &mut self::Cpu) {
        res_c(cpu, 5);
    }

    /// Clear `C` bit 6
    fn res_c_6(cpu: &mut self::Cpu) {
        res_c(cpu, 6);
    }

    /// Clear `C` bit 7
    fn res_c_7(cpu: &mut self::Cpu) {
        res_c(cpu, 7);
    }

    /// Helper function to clear bits in `D`
    fn res_d(cpu: &mut self::Cpu, bit: u8) {
        let d = cpu.d();
    ...
    /// Clear `D` bit 0
    fn res_d_0(cpu: &mut self::Cpu) {
        res_d(cpu, 0);
    }

    /// Clear `D` bit 1
    fn res_d_1(cpu: &mut self::Cpu) {
        res_d(cpu, 1);
    }

    /// Clear `D` bit 2
    fn res_d_2(cpu: &mut self::Cpu) {
        res_d(cpu, 2);
    }

    /// Clear `D` bit 3
    fn res_d_3(cpu: &mut self::Cpu) {
        res_d(cpu, 3);
    }

    /// Clear `D` bit 4
    fn res_d_4(cpu: &mut self::Cpu) {
        res_d(cpu, 4);
    }

    /// Clear `D` bit 5
    fn res_d_5(cpu: &mut self::Cpu) {
        res_d(cpu, 5);
    }

    /// Clear `D` bit 6
    fn res_d_6(cpu: &mut self::Cpu) {
        res_d(cpu, 6);
    }

    /// Clear `D` bit 7
    fn res_d_7(cpu: &mut self::Cpu) {
        res_d(cpu, 7);
    }

    /// Helper function to clear bits in `E`
    fn res_e(cpu: &mut self::Cpu, bit: u8) {
        let e = cpu.e();
    ...
    /// Clear `E` bit 0
    fn res_e_0(cpu: &mut self::Cpu) {
        res_e(cpu, 0);
    }

    /// Clear `E` bit 1
    fn res_e_1(cpu: &mut self::Cpu) {
        res_e(cpu, 1);
    }

    /// Clear `E` bit 2
    fn res_e_2(cpu: &mut self::Cpu) {
        res_e(cpu, 2);
    }

    /// Clear `E` bit 3
    fn res_e_3(cpu: &mut self::Cpu) {
        res_e(cpu, 3);
    }

    /// Clear `E` bit 4
    fn res_e_4(cpu: &mut self::Cpu) {
        res_e(cpu, 4);
    }

    /// Clear `E` bit 5
    fn res_e_5(cpu: &mut self::Cpu) {
        res_e(cpu, 5);
    }

    /// Clear `E` bit 6
    fn res_e_6(cpu: &mut self::Cpu) {
        res_e(cpu, 6);
    }

    /// Clear `E` bit 7
    fn res_e_7(cpu: &mut self::Cpu) {
        res_e(cpu, 7);
    }

    /// Helper function to clear bits in `H`
    fn res_h(cpu: &mut self::Cpu, bit: u8) {
        let h = cpu.h();
    ...
    /// Clear `H` bit 0
    fn res_h_0(cpu: &mut self::Cpu) {
        res_h(cpu, 0);
    }

    /// Clear `H` bit 1
    fn res_h_1(cpu: &mut self::Cpu) {
        res_h(cpu, 1);
    }

    /// Clear `H` bit 2
    fn res_h_2(cpu: &mut self::Cpu) {
        res_h(cpu, 2);
    }

    /// Clear `H` bit 3
    fn res_h_3(cpu: &mut self::Cpu) {
        res_h(cpu, 3);
    }

    /// Clear `H` bit 4
    fn res_h_4(cpu: &mut self::Cpu) {
        res_h(cpu, 4);
    }

    /// Clear `H` bit 5
    fn res_h_5(cpu: &mut self::Cpu) {
        res_h(cpu, 5);
    }

    /// Clear `H` bit 6
    fn res_h_6(cpu: &mut self::Cpu) {
        res_h(cpu, 6);
    }

    /// Clear `H` bit 7
    fn res_h_7(cpu: &mut self::Cpu) {
        res_h(cpu, 7);
    }

    /// Helper function to clear bits in `L`
    fn res_l(cpu: &mut self::Cpu, bit: u8) {
        let l = cpu.l();
    ...
    /// Clear `L` bit 0
    fn res_l_0(cpu: &mut self::Cpu) {
        res_l(cpu, 0);
    }

    /// Clear `L` bit 1
    fn res_l_1(cpu: &mut self::Cpu) {
        res_l(cpu, 1);
    }

    /// Clear `L` bit 2
    fn res_l_2(cpu: &mut self::Cpu) {
        res_l(cpu, 2);
    }

    /// Clear `L` bit 3
    fn res_l_3(cpu: &mut self::Cpu) {
        res_l(cpu, 3);
    }

    /// Clear `L` bit 4
    fn res_l_4(cpu: &mut self::Cpu) {
        res_l(cpu, 4);
    }

    /// Clear `L` bit 5
    fn res_l_5(cpu: &mut self::Cpu) {
        res_l(cpu, 5);
    }

    /// Clear `L` bit 6
    fn res_l_6(cpu: &mut self::Cpu) {
        res_l(cpu, 6);
    }

    /// Clear `L` bit 7
    fn res_l_7(cpu: &mut self::Cpu) {
        res_l(cpu, 7);
    }

    /// Helper function to clear bits in `[HL]`
    fn res_mhl(cpu: &mut self::Cpu, bit: u8) {
        let hl = cpu.hl();
    ...
    /// Clear `[HL]` bit 0
    fn res_mhl_0(cpu: &mut self::Cpu) {
        res_mhl(cpu, 0);
    }

    /// Clear `[HL]` bit 1
    fn res_mhl_1(cpu: &mut self::Cpu) {
        res_mhl(cpu, 1);
    }

    /// Clear `[HL]` bit 2
    fn res_mhl_2(cpu: &mut self::Cpu) {
        res_mhl(cpu, 2);
    }

    /// Clear `[HL]` bit 3
    fn res_mhl_3(cpu: &mut self::Cpu) {
        res_mhl(cpu, 3);
    }

    /// Clear `[HL]` bit 4
    fn res_mhl_4(cpu: &mut self::Cpu) {
        res_mhl(cpu, 4);
    }

    /// Clear `[HL]` bit 5
    fn res_mhl_5(cpu: &mut self::Cpu) {
        res_mhl(cpu, 5);
    }

    /// Clear `[HL]` bit 6
    fn res_mhl_6(cpu: &mut self::Cpu) {
        res_mhl(cpu, 6);
    }

    /// Clear `[HL]` bit 7
    fn res_mhl_7(cpu: &mut self::Cpu) {
        res_mhl(cpu, 7);
    }
    ...
    fn set_a(cpu: &mut self::Cpu, bit: u8) {
        let a = cpu.a();
    ...
    /// Set `A` bit 0
    fn set_a_0(cpu: &mut self::Cpu) {
        set_a(cpu, 0);
    }

    /// Set `A` bit 1
    fn set_a_1(cpu: &mut self::Cpu) {
        set_a(cpu, 1);
    }

    /// Set `A` bit 2
    fn set_a_2(cpu: &mut self::Cpu) {
        set_a(cpu, 2);
    }

    /// Set `A` bit 3
    fn set_a_3(cpu: &mut self::Cpu) {
        set_a(cpu, 3);
    }

    /// Set `A` bit 4
    fn set_a_4(cpu: &mut self::Cpu) {
        set_a(cpu, 4);
    }

    /// Set `A` bit 5
    fn set_a_5(cpu: &mut self::Cpu) {
        set_a(cpu, 5);
    }

    /// Set `A` bit 6
    fn set_a_6(cpu: &mut self::Cpu) {
        set_a(cpu, 6);
    }

    /// Set `A` bit 7
    fn set_a_7(cpu: &mut self::Cpu) {
        set_a(cpu, 7);
    }

    /// Helper function to set bits in `B`
    fn set_b(cpu: &mut self::Cpu, bit: u8) {
        let b = cpu.b();
    ...
    /// Set `B` bit 0
    fn set_b_0(cpu: &mut self::Cpu) {
        set_b(cpu, 0);
    }

    /// Set `B` bit 1
    fn set_b_1(cpu: &mut self::Cpu) {
        set_b(cpu, 1);
    }

    /// Set `B` bit 2
    fn set_b_2(cpu: &mut self::Cpu) {
        set_b(cpu, 2);
    }

    /// Set `B` bit 3
    fn set_b_3(cpu: &mut self::Cpu) {
        set_b(cpu, 3);
    }

    /// Set `B` bit 4
    fn set_b_4(cpu: &mut self::Cpu) {
        set_b(cpu, 4);
    }

    /// Set `B` bit 5
    fn set_b_5(cpu: &mut self::Cpu) {
        set_b(cpu, 5);
    }

    /// Set `B` bit 6
    fn set_b_6(cpu: &mut self::Cpu) {
        set_b(cpu, 6);
    }

    /// Set `B` bit 7
    fn set_b_7(cpu: &mut self::Cpu) {
        set_b(cpu, 7);
    }

    /// Helper function to set bits in `C`
    fn set_c(cpu: &mut self::Cpu, bit: u8) {
        let c = cpu.c();
    ...
    /// Set `C` bit 0
    fn set_c_0(cpu: &mut self::Cpu) {
        set_c(cpu, 0);
    }

    /// Set `C` bit 1
    fn set_c_1(cpu: &mut self::Cpu) {
        set_c(cpu, 1);
    }

    /// Set `C` bit 2
    fn set_c_2(cpu: &mut self::Cpu) {
        set_c(cpu, 2);
    }

    /// Set `C` bit 3
    fn set_c_3(cpu: &mut self::Cpu) {
        set_c(cpu, 3);
    }

    /// Set `C` bit 4
    fn set_c_4(cpu: &mut self::Cpu) {
        set_c(cpu, 4);
    }

    /// Set `C` bit 5
    fn set_c_5(cpu: &mut self::Cpu) {
        set_c(cpu, 5);
    }

    /// Set `C` bit 6
    fn set_c_6(cpu: &mut self::Cpu) {
        set_c(cpu, 6);
    }

    /// Set `C` bit 7
    fn set_c_7(cpu: &mut self::Cpu) {
        set_c(cpu, 7);
    }

    /// Helper function to set bits in `D`
    fn set_d(cpu: &mut self::Cpu, bit: u8) {
        let d = cpu.d();
    ...
    /// Set `D` bit 0
    fn set_d_0(cpu: &mut self::Cpu) {
        set_d(cpu, 0);
    }

    /// Set `D` bit 1
    fn set_d_1(cpu: &mut self::Cpu) {
        set_d(cpu, 1);
    }

    /// Set `D` bit 2
    fn set_d_2(cpu: &mut self::Cpu) {
        set_d(cpu, 2);
    }

    /// Set `D` bit 3
    fn set_d_3(cpu: &mut self::Cpu) {
        set_d(cpu, 3);
    }

    /// Set `D` bit 4
    fn set_d_4(cpu: &mut self::Cpu) {
        set_d(cpu, 4);
    }

    /// Set `D` bit 5
    fn set_d_5(cpu: &mut self::Cpu) {
        set_d(cpu, 5);
    }

    /// Set `D` bit 6
    fn set_d_6(cpu: &mut self::Cpu) {
        set_d(cpu, 6);
    }

    /// Set `D` bit 7
    fn set_d_7(cpu: &mut self::Cpu) {
        set_d(cpu, 7);
    }

    /// Helper function to set bits in `E`
    fn set_e(cpu: &mut self::Cpu, bit: u8) {
        let e = cpu.e();
    ...
    /// Set `E` bit 0
    fn set_e_0(cpu: &mut self::Cpu) {
        set_e(cpu, 0);
    }

    /// Set `E` bit 1
    fn set_e_1(cpu: &mut self::Cpu) {
        set_e(cpu, 1);
    }

    /// Set `E` bit 2
    fn set_e_2(cpu: &mut self::Cpu) {
        set_e(cpu, 2);
    }

    /// Set `E` bit 3
    fn set_e_3(cpu: &mut self::Cpu) {
        set_e(cpu, 3);
    }

    /// Set `E` bit 4
    fn set_e_4(cpu: &mut self::Cpu) {
        set_e(cpu, 4);
    }

    /// Set `E` bit 5
    fn set_e_5(cpu: &mut self::Cpu) {
        set_e(cpu, 5);
    }

    /// Set `E` bit 6
    fn set_e_6(cpu: &mut self::Cpu) {
        set_e(cpu, 6);
    }

    /// Set `E` bit 7
    fn set_e_7(cpu: &mut self::Cpu) {
        set_e(cpu, 7);
    }

    /// Helper function to set bits in `H`
    fn set_h(cpu: &mut self::Cpu, bit: u8) {
        let h = cpu.h();
    ...
    /// Set `H` bit 0
    fn set_h_0(cpu: &mut self::Cpu) {
        set_h(cpu, 0);
    }

    /// Set `H` bit 1
    fn set_h_1(cpu: &mut self::Cpu) {
        set_h(cpu, 1);
    }

    /// Set `H` bit 2
    fn set_h_2(cpu: &mut self::Cpu) {
        set_h(cpu, 2);
    }

    /// Set `H` bit 3
    fn set_h_3(cpu: &mut self::Cpu) {
        set_h(cpu, 3);
    }

    /// Set `H` bit 4
    fn set_h_4(cpu: &mut self::Cpu) {
        set_h(cpu, 4);
    }

    /// Set `H` bit 5
    fn set_h_5(cpu: &mut self::Cpu) {
        set_h(cpu, 5);
    }

    /// Set `H` bit 6
    fn set_h_6(cpu: &mut self::Cpu) {
        set_h(cpu, 6);
    }

    /// Set `H` bit 7
    fn set_h_7(cpu: &mut self::Cpu) {
        set_h(cpu, 7);
    }

    /// Helper function to set bits in `L`
    fn set_l(cpu: &mut self::Cpu, bit: u8) {
        let l = cpu.l();
    ...
    /// Set `L` bit 0
    fn set_l_0(cpu: &mut self::Cpu) {
        set_l(cpu, 0);
    }

    /// Set `L` bit 1
    fn set_l_1(cpu: &mut self::Cpu) {
        set_l(cpu, 1);
    }

    /// Set `L` bit 2
    fn set_l_2(cpu: &mut self::Cpu) {
        set_l(cpu, 2);
    }

    /// Set `L` bit 3
    fn set_l_3(cpu: &mut self::Cpu) {
        set_l(cpu, 3);
    }

    /// Set `L` bit 4
    fn set_l_4(cpu: &mut self::Cpu) {
        set_l(cpu, 4);
    }

    /// Set `L` bit 5
    fn set_l_5(cpu: &mut self::Cpu) {
        set_l(cpu, 5);
    }

    /// Set `L` bit 6
    fn set_l_6(cpu: &mut self::Cpu) {
        set_l(cpu, 6);
    }

    /// Set `L` bit 7
    fn set_l_7(cpu: &mut self::Cpu) {
        set_l(cpu, 7);
    }

    /// Helper function to set bits in `[HL]`
    fn set_mhl(cpu: &mut self::Cpu, bit: u8) {
        let hl = cpu.hl();
    ...
    /// Set `[HL]` bit 0
    fn set_mhl_0(cpu: &mut self::Cpu) {
        set_mhl(cpu, 0);
    }

    /// Set `[HL]` bit 1
    fn set_mhl_1(cpu: &mut self::Cpu) {
        set_mhl(cpu, 1);
    }

    /// Set `[HL]` bit 2
    fn set_mhl_2(cpu: &mut self::Cpu) {
        set_mhl(cpu, 2);
    }

    /// Set `[HL]` bit 3
    fn set_mhl_3(cpu: &mut self::Cpu) {
        set_mhl(cpu, 3);
    }

    /// Set `[HL]` bit 4
    fn set_mhl_4(cpu: &mut self::Cpu) {
        set_mhl(cpu, 4);
    }

    /// Set `[HL]` bit 5
    fn set_mhl_5(cpu: &mut self::Cpu) {
        set_mhl(cpu, 5);
    }

    /// Set `[HL]` bit 6
    fn set_mhl_6(cpu: &mut self::Cpu) {
        set_mhl(cpu, 6);
    }

    /// Set `[HL]` bit 7
    fn set_mhl_7(cpu: &mut self::Cpu) {
        set_mhl(cpu, 7);
    }
